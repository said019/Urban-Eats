...
import {
  CATALOG, RAMEN_FOR_REWARD, STORAGE_KEYS, isRamenProduct, getProduct, getAllItems,
...
export default function POSPage() {
  const [activeCat, setActiveCat] = useState<string>(Object.keys(CATALOG)[0]);
  ...
  const [lastSale, setLastSale] = useState<(Sale & { customer?: ApiClient | null }) | null>(null);
  ...
  useEffect(() => {
    ...
    setSales(loadJson(STORAGE_KEYS.SALES, []));
  }, []);
  ...
  const addToCart = (id: string) => {
    ...
    if (!product.isService && currentStock <= 0) return;
    const existing = cart.find((c) => c.id === id);
    if (existing) {
      if (!product.isService && existing.qty >= currentStock) return;
      ...
      setCart([
        ...cart,
        {
          ...
          name: product.name,
          cost: product.cost,
          price: getDisplayPrice(id, product.price),
          qty: 1,
  ...
  const updateQty = (id: string, delta: number) => {
    ...
    const product = getProduct(id);
    if (product && !product.isService && newQty > (stock[id] ?? 0)) return;
  ...
  const ramenInCart = cart.filter((c) => isRamenProduct(c.id));
  ...
  const ramenQty = ramenInCart.reduce((s, c) => s + c.qty, 0);
  ...
  const finalize = async (method: string) => {
    ...
    cart.forEach((c) => { if (!c.isService) newStock[c.id] = (newStock[c.id] ?? 0) - c.qty; });

    // 2. Save sale locally
    const sale: Sale = {
      id: Date.now(),
      date: new Date().toISOString(),
      items: cart.map((c) => ({ id: c.id, name: c.name, qty: c.qty, price: c.price, cost: c.cost })),
      subtotal, discount, total, profit, method,
    ...
    setStock(newStock); setSales(newSales);
    saveJson(STORAGE_KEYS.STOCK, newStock);
    saveJson(STORAGE_KEYS.SALES, newSales);

    // 3. Sync stamps to backend (one POST per ramen → triggers wallet push)
    let updatedCustomer: ApiClient | null = currentCustomer;
    if (currentCustomer && (ramenQty > 0 || (useReward && canUseReward))) {
      const token = localStorage.getItem('admin_token');

      // Si canjea recompensa: redeem (resetea sellos)
      if (useReward && canUseReward) {
        try {
          const r = await fetch(`/api/loyalty/clients/${currentCustomer.id}/redeem`, {
            method: 'POST',
          ...
          if (r.ok) {
            ...
            updatedCustomer = { ...currentCustomer, stamps: data.newStamps ?? 0 };
          }
        } catch {}
      }

      // Sumar 1 sello por cada ramen en el carrito
      for (let i = 0; i < ramenQty; i++) {
        try {
          const r = await fetch(`/api/admin/stamp/${currentCustomer.id}`, {
            method: 'POST',
            headers: { Authorization: `Bearer ${token}` },
          });
          if (r.ok) {
            const data = await r.json();
            updatedCustomer = updatedCustomer ? { ...updatedCustomer, stamps: data.newStamps } : updatedCustomer;
          }
        } catch {}
      }
    }

    setLastSale({ ...sale, customer: updatedCustomer });
    setCart([]); setCurrentCustomer(null); setUseReward(false); setShowCheckout(false);
    setProcessing(false);
  };

  const todaySales = useMemo(() => sales.filter(
  ...
  const visibleItems = search
    ...
    : CATALOG[activeCat].items.map((i) => ({ ...i, _cat: CATALOG[activeCat] }));
    ...
    <div className="-m-6 sm:-m-10 min-h-[calc(100vh-4rem)] bg-gradient-to-br from-pink-50 via-white to-rose-50 text-gray-800">
      <div className="px-4 py-3 sticky top-0 z-20 bg-white/95 backdrop-blur border-b-4 border-pink-500 flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-pink-50 hover:bg-pink-100 text-pink-700 px-3 py-2 rounded-lg font-semibold text-sm transition"
        >
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-10 py-3 bg-white border-2 border-pink-100 rounded-xl focus:border-pink-400 focus:outline-none text-gray-800"
            />
            {search && (
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 -mx-1 px-1">
              {Object.entries(CATALOG).map(([key, cat]) => (
                <button
                  ...
                  className={`whitespace-nowrap px-4 py-2.5 rounded-xl font-bold text-sm transition-all ${activeCat === key ? 'text-white shadow-md scale-105' : 'bg-white text-gray-600 hover:bg-pink-50 border-2 border-pink-100'}`}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {visibleItems.map((item) => {
              ...
              const isOut = !item.isService && itemStock <= 0;
              ...
              return (
                <button
                  ...
                  className={`relative text-left bg-white rounded-2xl p-3 shadow-sm border-2 transition-all hover:shadow-md hover:-translate-y-0.5 ${isOut ? 'opacity-40 cursor-not-allowed' : inCart ? 'border-pink-400' : 'border-transparent'}`}
                >
                  {inCart && (
                    <div className="absolute -top-2 -right-2 bg-pink-500 text-white text-xs font-bold rounded-full w-7 h-7 flex items-center justify-center shadow-md">
                      {inCart.qty}
                    </div>
                  )}
                  <div className="h-16 rounded-xl mb-2 flex items-center justify-center text-white font-black text-2xl" style={{ backgroundColor: cat.color }}>
                    {item.isService ? '🍜' : item.name.charAt(0)}
                  </div>
                  <div className="text-xs font-semibold text-gray-800 leading-tight mb-1 line-clamp-2 min-h-[2rem]">{item.name}</div>
                  <div className="flex items-center justify-between">
                    <span className="text-lg font-black text-pink-600">${displayPrice}</span>
                    {!item.isService && (
                      <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${itemStock <= 3 ? 'bg-red-100 text-red-700' : itemStock <= 8 ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700'}`}>
                        {itemStock}
                      </span>
                    )}
                  </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-gray-500">
                    ...
                    {currentCustomer.stamps >= RAMEN_FOR_REWARD && (
                      <span className="bg-green-100 text-green-700 font-bold px-1.5 py-0.5 rounded text-[10px] ml-1">¡RECOMPENSA!</span>
                    )}
                  </div>
                </div>
                <button onClick={() => { setCurrentCustomer(null); setUseReward(false); }} className="text-gray-400 hover:text-gray-600 p-1">
                  <X size={16} />
                </button>
              </div>
            ) : (
                  ...
                  {cart.map((item) => (
                    <div key={item.id} className="p-3 flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-1 bg-pink-50 rounded-lg p-1">
                        <button onClick={() => updateQty(item.id, -1)} className="w-7 h-7 rounded bg-white text-pink-600 hover:bg-pink-100 flex items-center justify-center">
                          <Minus size={14} />
                        </button>
            ...
            {cart.length > 0 && (
              <div className="border-t-2 border-pink-100 p-4 space-y-2 bg-pink-50/50">
                ...
                <button
                  ...
                  className="w-full bg-gradient-to-r from-pink-500 to-rose-500 hover:from-pink-600 hover:to-rose-600 text-white font-bold py-3 rounded-xl shadow-md transition-all"
                >
      ...
      {showCustomerLookup && (
        <CustomerLookup
          onSelect={(c) => { setCurrentCustomer(c); setShowCustomerLookup(false); }}
          ...
          <div className="bg-white rounded-2xl max-w-md w-full overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="bg-gradient-to-r from-pink-500 to-rose-500 p-5 text-white text-center">
              ...
              <div className="text-xs opacity-75">
                {cart.reduce((s, c) => s + c.qty, 0)} productos{currentCustomer && ` · ${currentCustomer.name}`}
              </div>
            </div>
            <div className="p-5 space-y-3">
              ...
              <button disabled={processing} onClick={() => setShowCheckout(false)} className="w-full text-gray-500 font-medium py-2 hover:text-gray-700">
                Cancelar
              </button>
            </div>
          </div>
      ...
      {lastSale && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setLastSale(null)}>
          <div className="bg-white rounded-2xl max-w-md w-full p-6 text-center" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="text-sm text-gray-500 mb-3">{lastSale.method} · Margen: ${lastSale.profit.toFixed(0)}</div>
            {lastSale.customer && (
              <div className="bg-pink-50 rounded-xl p-3 mb-4">
                <div className="text-xs font-bold text-pink-700 mb-2">{lastSale.customer.name}</div>
                <div className="text-pink-700 font-bold">
                  {lastSale.customer.stamps} / {RAMEN_FOR_REWARD} sellos
                </div>
                {lastSale.rewardUsed && (
            ...
            <button onClick={() => setLastSale(null)} className="w-full bg-pink-500 hover:bg-pink-600 text-white font-bold py-3 rounded-xl">
              Continuar
            </button>
          </div>
        </div>
                    ...
                    <div key={s.id} className="p-3 flex items-center gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-semibold flex items-center gap-2 flex-wrap">
                          ...
                          <span className="text-xs text-gray-400 font-normal">{s.method}</span>
                          {s.customerName && (
                            <span className="text-xs text-pink-600 font-bold flex items-center gap-1">
                              <User size={10} />{s.customerName}
                            </span>
                          )}
                        </div>
                        <div className="text-xs text-gray-500 truncate">{s.items.map((i) => `${i.qty}× ${i.name}`).join(', ')}</div>
                      </div>
                      <div className="text-right">
                        <div className="font-bold text-pink-600">${s.total.toFixed(0)}</div>
                        <div className="text-[10px] text-green-600 font-bold">+${s.profit.toFixed(0)}</div>
                      </div>
                    </div>
                  ))}
                </div>
...
function CustomerLookup({ onSelect, onClose }: { onSelect: (c: ApiClient) => void; onClose: () => void }) {
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      ...
      try {
        const token = localStorage.getItem('admin_token');
        const r = await fetch(`/api/admin/clients-list?search=${encodeURIComponent(search)}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
            ...
            <div className="p-3 border-b">
              <div className="relative">
                ...
                <input
                  ...
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                  className="w-full pl-10 pr-3 py-2.5 border-2 border-pink-100 rounded-xl focus:border-pink-400 focus:outline-none text-sm"
                />
              </div>
            </div>
            <div className="overflow-y-auto flex-1">
