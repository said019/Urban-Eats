  ...
  const handleGoogleWallet = async () => {
    ...
    try {
      ...
      if (data.saveUrl) {
        ...
        alert(data.error || 'Error generando el pase de Google Wallet');
      }
    } catch {
    ...
    <div className="min-h-screen relative flex flex-col items-center pt-12 px-6 pb-20 justify-between">
      ...
      >
        ...
        <button
          ...
          disabled={!dbClientId || googleWalletLoading}
          className={`w-full py-4 rounded-full border border-zinc-700 bg-black/40 backdrop-blur-md flex items-center justify-center gap-3 text-sm font-bold text-white transition-colors ${dbClientId && !googleWalletLoading ? 'hover:border-brand-yellow cursor-pointer' : 'opacity-50 cursor-not-allowed'}`}
        >
          ...
          {googleWalletLoading ? 'Generando...' : 'Add to Google Wallet'}
        </button>
      </motion.div>
